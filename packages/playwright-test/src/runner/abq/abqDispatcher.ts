...
export class AbqDispatcher extends Dispatcher {
  ...
  protected override async _scheduleJob() {
    ...
    const testCase = (await Abq.protocolRead(
      this._abqSocket
    )) as Abq.TestCaseMessage;
        ...
        status: ((result: TestResult): Abq.TestResultStatus => {
          switch (result.status) {
            ...
            case "skipped":
              return { type: "skipped" };
            case "failed":
              return { type: "failure" }; // TODO find exception & backtrace if possible
            case "interrupted":
              return { type: "error" }; // TODO find exception & backtrace if possible
          }
        })(result),
        id: test.id,
